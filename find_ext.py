...
@click.command()
@click.option('--inpath', '-p', default=os.getcwd(), help='Path to search for files - defaults = programs directory')
@click.option('--outpath', '-op', help='Path to move/copy files to')
@click.option('--ext', '-e', help='File extension to look for')
@click.option('--operation', '-o', default='mv', help='Operation to use - mv, cp, or rm, default = mv')
@click.option('--recursive', '-r', default=False, help='Search for files in recursive subdirs - default =False')
def find_ext(inpath, outpath, ext, operation, recursive):
    if operation == 'cp':
